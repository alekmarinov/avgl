										 ...
										 av_rect_p arect, av_rect_p dstrect, av_rect_p srcrect)
{
	...
	_remainder = srcrect->x % ctx->gridsize;
	srect.x = srcrect->x - _remainder;
	srect.w = srcrect->w + _remainder;
	_remainder = srect.w % ctx->gridsize;
	if (_remainder > 0)
		srect.w += (ctx->gridsize - _remainder);
	/* FIXME: check srect.w < MAX_SRC_WIDTH */
	
	_remainder = srcrect->y % ctx->gridsize;
	srect.y = srcrect->y - _remainder;
	srect.h = srcrect->h + _remainder;
	_remainder = srect.h % ctx->gridsize;
	if (_remainder > 0)
		srect.h += (ctx->gridsize - _remainder);
	/* FIXME: check srect.h < MAX_SRC_HEIGHT */
	
	_remainder = dstrect->x % ctx->scaledgridsize;
	drect.x = dstrect->x - _remainder;
	drect.w = dstrect->w + _remainder;
	_remainder = drect.w % ctx->scaledgridsize;
	if (_remainder > 0)
		drect.w += (ctx->scaledgridsize - _remainder);
	/* FIXME: check drect.w < MAX_DST_WIDTH */

	_remainder = dstrect->y % ctx->scaledgridsize;
	drect.y = dstrect->y - _remainder;
	drect.h = dstrect->h + _remainder;
	_remainder = drect.h % ctx->scaledgridsize;
	if (_remainder > 0)
		drect.h += (ctx->scaledgridsize - _remainder);
...
static av_bool_t av_xembed_window_on_mouse_move(av_window_p self, int x, int y)
{
	...
	if (ctx->self->on_mouse_event)
		ctx->self->on_mouse_event(ctx->self, "move", x, y);
	return AV_TRUE;
}
										 ...
										 av_rect_p rect)
{
	...
	ctx->window->set_visible(ctx->window, AV_FALSE);
	ctx->window->set_cursor_visible(ctx->window, AV_FALSE);
...
static void av_xembed_window_set_mouse_visible(av_xembed_window_p self, av_bool_t mouse_visible)
{
	av_xembed_window_ctx_p ctx = (av_xembed_window_ctx_p)O_context(self);
	ctx->xmouse_visible = mouse_visible;
}

static void av_xembed_window_send_mouse_button(av_xembed_window_p self,
											   av_event_mouse_button_t button,
											   av_event_button_status_t status)
{
	av_xembed_window_ctx_p ctx = (av_xembed_window_ctx_p)O_context(self);
	ctx->xembed->send_mouse_button(ctx->xembed, button, status);
}

static void av_xembed_window_send_mouse_move(av_xembed_window_p self, int x, int y)
{
	av_xembed_window_ctx_p ctx = (av_xembed_window_ctx_p)O_context(self);	
	av_rect_t arect;
	ctx->window->get_rect(ctx->window, &arect);
	...
	av_xembed_window_invalidate_mouse(ctx, ctx->last_mouse_x, ctx->last_mouse_y);
...
static void av_xembed_window_set_mouse_capture(av_xembed_window_p self, av_bool_t capture)
{
	av_xembed_window_ctx_p ctx = (av_xembed_window_ctx_p)O_context(self);
	if (capture)
		ctx->window->capture(ctx->window);
	else
		ctx->window->uncapture(ctx->window);
}

static void av_xembed_window_set_cursor(av_xembed_window_p self, av_video_cursor_shape_t cursor)
{
	av_xembed_window_ctx_p ctx = (av_xembed_window_ctx_p)O_context(self);
	ctx->window->set_cursor(ctx->window, cursor);
}

static void av_xembed_window_set_cursor_visible(av_xembed_window_p self, av_bool_t cursor_visible)
{
	int last_mx, last_my;
	av_xembed_window_ctx_p ctx = (av_xembed_window_ctx_p)O_context(self);

	/* remember last mouse position */
	ctx->video->get_mouse_position(ctx->video, &last_mx, &last_my);

	/* set window attribute cursor visibility */
	ctx->window->set_cursor_visible(ctx->window, cursor_visible);

	/* change cursor visibility now */
	ctx->video->set_cursor_visible(ctx->video, cursor_visible);

	/* change X cursor visibility the oposite of the AVGL cursor */
	self->set_xcursor_visible(self, !cursor_visible);

	ctx->video->set_mouse_position(ctx->video, last_mx, last_my);
}

static void av_xembed_window_set_xcursor_visible(av_xembed_window_p self, av_bool_t cursor_visible)
{
	av_xembed_window_ctx_p ctx = (av_xembed_window_ctx_p)O_context(self);

	/* change X cursor visibility */
	ctx->xembed->set_cursor_visible(ctx->xembed, cursor_visible);
}

static void av_xembed_window_focus(av_xembed_window_p self)
{
	av_xembed_window_ctx_p ctx = (av_xembed_window_ctx_p)O_context(self);
	ctx->system->set_focus(ctx->system, ctx->window);
}

static av_bool_t av_xembed_window_is_focus(av_xembed_window_p self)
{
	av_xembed_window_ctx_p ctx = (av_xembed_window_ctx_p)O_context(self);
	return ctx->system->is_focus(ctx->system, ctx->window);
}
...
static av_result_t av_xembed_window_constructor(av_object_p pobject)
{
	...
	self->set_mouse_visible  = av_xembed_window_set_mouse_visible;
	self->send_mouse_button  = av_xembed_window_send_mouse_button;
	self->send_mouse_move    = av_xembed_window_send_mouse_move;
	self->set_mouse_capture  = av_xembed_window_set_mouse_capture;
	self->set_cursor         = av_xembed_window_set_cursor;
	self->set_cursor_visible = av_xembed_window_set_cursor_visible;
	self->set_xcursor_visible= av_xembed_window_set_xcursor_visible;
	self->focus              = av_xembed_window_focus;
	self->is_focus           = av_xembed_window_is_focus;
