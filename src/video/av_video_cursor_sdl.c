...
/*                                                                   */
/* Project:       avgl                                               */
/* Filename:      av_video_cursor_sdl.c                              */
...
void av_video_cursor_sdl_init()
{
	av_video_cursor_sdl_done();
	if (!initialized)
	{
		cursors[AV_VIDEO_CURSOR_DEFAULT]  = create_cursor_from_xpm(arrow_xpm, 6,  0);
		cursors[AV_VIDEO_CURSOR_BEAM]     = create_cursor_from_xpm(beam_xpm, 6, 0);
		cursors[AV_VIDEO_CURSOR_WAIT]     = create_cursor_from_xpm(busy_xpm, 6, 0);
		cursors[AV_VIDEO_CURSOR_HAND]     = create_cursor_from_xpm(hand_xpm,  6, 0);
		cursors[AV_VIDEO_CURSOR_KEYBOARD] = create_cursor_from_xpm(keyboard_xpm,  6, 0);
		cursors[AV_VIDEO_CURSOR_ZOOM]     = create_cursor_from_xpm(zoom_xpm,  6, 0);
		cursors[AV_VIDEO_CURSOR_SCROLL]   = create_cursor_from_xpm(scroll_xpm,  6, 0);
...
{
	if (cursor_shape < AV_VIDEO_CURSOR_LAST)
	{
...
void av_video_cursor_sdl_done(void)
{
	...
	{
		int i;
		for (i=0; i<AV_VIDEO_CURSOR_LAST; i++)
		{
