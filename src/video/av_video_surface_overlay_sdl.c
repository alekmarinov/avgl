...
/*                                                                   */
/* Project:       avgl                                               */
/* Filename:      av_video_surface_overlay_sdl.c                     */
...
static av_result_t av_video_overlay_sdl_blit(struct av_video_overlay* self, av_rect_p dstrect)
{
	...
	if (ctx->mtx)
	{
		...
		if (ctx->overlay)
		{
			...
			{
				rect.x = ctx->dstrect.x; rect.y = ctx->dstrect.y;
				rect.w = ctx->dstrect.w; rect.h = ctx->dstrect.h;
			}
...
static av_result_t av_video_overlay_sdl_blit_back(struct av_video_overlay* self,
												  av_rect_p srcrect, av_rect_p dstrect)
{
	av_video_overlay_sdl_p ctx = O_context_overlay(self);
	AV_UNUSED(srcrect);
	if (ctx->mtx)
														 ...
														 av_video_overlay_format_t format)
{
	...
	if (ctx->mtx)
	{
		...
		if (!ctx->surface)
		{
			...
			if (AV_OK != self->video->get_backbuffer(self->video, &backbuffer))
			{
				ctx->mtx->unlock(ctx->mtx);
				return AV_EMEM;
			}
			ctx->surface = O_context(backbuffer);
		}
...
static av_result_t av_video_overlay_sdl_set_size_back(struct av_video_overlay* self, int back_width, int back_height)
{
	...
	{
		ctx->mtx->lock(ctx->mtx);
		ctx->dstrect.w = back_width;
		ctx->dstrect.h = back_height;
		ctx->mtx->unlock(ctx->mtx);
	}
	return AV_OK;
}
